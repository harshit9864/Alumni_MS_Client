...
import { Menu, LayoutDashboard, Calendar, Search, FileText } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-violet-200 flex">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-50 border-r border-slate-200 min-h-screen sticky top-0 py-6 px-4">
        {/* Logo */}
        <Link href="/student" className="flex items-center gap-2 group mb-8 px-2">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="md:hidden sticky top-0 z-50 w-full border-b border-zinc-200 bg-white/80 backdrop-blur-md flex items-center justify-between px-4 h-16">
          ...
          <Link href="/student" className="flex items-center gap-2 group">
            <div className="w-8 h-8 bg-violet-600 rounded-lg flex items-center justify-center text-white font-bold shadow-md shadow-violet-200">
              A
            </div>
            <span className="text-xl font-bold text-zinc-900 tracking-tight">
              Alumni<span className="text-violet-600">Connect</span>
...
function NavLinks({ 
  ...
  const links = [
    ...
    { name: "Find Alumni", href: "/student/findalumni", icon: Search },
    { name: "Blogs", href: "/student/blogs", icon: FileText },
  ];
